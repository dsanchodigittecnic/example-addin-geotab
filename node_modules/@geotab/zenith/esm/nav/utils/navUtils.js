...
export const filterPrimaryNavItems = (element, isNegative) => {
    ...
    if (!isNavItem && children) {
        const filteredChildren = Children.map(children, child => filterPrimaryNavItems(child, isNegative));
        if (!filteredChildren) {
            return null;
        }
    ...
    if (isNavItem) {
        ...
        if (children) {
            ...
            const validChildren = filteredChildren.filter(Boolean);
            return validChildren.length > 0 ? Object.assign(Object.assign({}, element), { props: Object.assign(Object.assign({}, element.props), { children: validChildren }) }) : element;
        }
        return element;
    }
    return element;
};
export const collectNavItems = (element) => {
    const isNavItem = element.type && element.type.displayName === "NavItem";
    ...
    return children.length > 0
        ? children.flatMap(child => collectNavItems(child))
        : [];
};
