import { addDays, addMinutes, getAvailablePeriod } from "../../../calendar/dateUtils";
...
const getAvailableRange = (availablePeriod, startDate, endDate, disableDatesBefore, disableDatesAfter) => {
    ...
    if (availablePeriod === "all") {
        fromDate = startDate ? startDate : endDate ? addDays(new Date(endDate), -1).toISOString() : now;
        toDate = endDate ? endDate : startDate ? addDays(new Date(startDate), 1).toISOString() : addDays(new Date(now), 1).toISOString();
    ...
    if (availablePeriod === "before" && disableDatesAfter) {
        ...
        fromDate = startDate && new Date(startDate) < new Date(disableDatesAfter) && new Date(startDate) < new Date(toDate) ? startDate : addDays(new Date(toDate), -1).toISOString();
    }
    if (availablePeriod === "after" && disableDatesBefore) {
        ...
        toDate = endDate && new Date(endDate) > new Date(disableDatesBefore) && new Date(endDate) > new Date(fromDate) ? endDate : addDays(new Date(fromDate), 1).toISOString();
    }
    if (availablePeriod === "outside" && disableDatesBefore && disableDatesAfter) {
        ...
        if (startDate && new Date(startDate) > new Date(disableDatesBefore)) {
            fromDate = startDate;
            toDate = endDate && new Date(endDate) > new Date(fromDate) ? endDate : addDays(new Date(fromDate), 1).toISOString();
            isAvailableRangeReady = true;
        }
        if (endDate && !isAvailableRangeReady && new Date(endDate) <= new Date(disableDatesAfter)) {
            toDate = endDate;
            fromDate = startDate && new Date(startDate) < new Date(toDate) ? startDate : addDays(new Date(toDate), -1).toISOString();
            isAvailableRangeReady = true;
        }
        if (endDate && !isAvailableRangeReady && new Date(endDate) > new Date(disableDatesBefore)) {
            ...
            fromDate = startDate && new Date(startDate) > new Date(disableDatesBefore) && new Date(startDate) < new Date(toDate) ? startDate
                : (addDays(new Date(toDate), -1) >= new Date(disableDatesBefore) ? addDays(new Date(toDate), -1).toISOString() : disableDatesBefore);
            isAvailableRangeReady = true;
        }
        if (!isAvailableRangeReady) {
            fromDate = disableDatesBefore;
            toDate = addDays(new Date(fromDate), 1).toISOString();
        }
    }
    if (availablePeriod === "inside" && disableDatesBefore && disableDatesAfter) {
        ...
        const fromDatePlusOne = addDays(new Date(fromDate), 1).toISOString();
...
// eslint-disable-next-line complexity
export const normalizeDates = (startDate, endDate, disableFutureDates, disablePastDates, disableDatesBefore, disableDatesAfter) => {
    const minuteReserve = 2;
    const now = new Date();
    const moreNow = addMinutes(new Date(now), minuteReserve);
    const availablePeriod = getAvailablePeriod(disableDatesBefore, disableDatesAfter);
    if (disableFutureDates && availablePeriod === "all") {
        const updatedStartDate = addDays(new Date(now), -1);
    ...
    if (disablePastDates && availablePeriod === "all") {
        const updatedEndDate = addDays(new Date(now), 1);
        ...
        if (startDate && new Date(startDate) >= moreNow) {
            return { from: startDate, to: endDate && new Date(endDate) > new Date(startDate) ? endDate : addDays(new Date(startDate), 1).toISOString() };
        }
        return { from: moreNow.toISOString(), to: endDate || updatedEndDate.toISOString() };
    }
