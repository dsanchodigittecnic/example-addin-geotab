...
export const calculateMinTruncationPrecision = (values) => {
    if (values.length === 0) {
        return 0;
    }
    ...
    if (uniqueValuesOriginal === 1) {
        const decimals = countDecimals(values[0]);
        return decimals < 3 ? decimals : 3;
    }
    for (let precision = 0; precision <= maxPrecision; precision++) {
        const truncatedValues = values.map(val => truncateDecimals(val, precision));
